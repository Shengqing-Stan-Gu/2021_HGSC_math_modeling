...
y0PDS = read.csv("./Expected y at diagnosis/yexpectedPDS rough.csv")[,-1]
y0NACT = read.csv("./Expected y at diagnosis/yexpectedNACT rough.csv")[,-1]
...
for (k in 1:length(epsilon)) {
  for (j in 1:length(u)) {
    ...
    write.csv(cbind(postPDSx,postPDSy,postNACTx,postNACTy,SurvivalPDS,ChemocountPDS,SurvivalNACT,ChemocountNACT),file=paste("./Parameter training rough/u",j," epsilon",k,".csv",sep=""))
  }
}
...
pdf("./Parameter training rough/Survival with u epsilon.pdf",width=12,height=10)
par(mfrow=c(length(epsilon),length(u)),mar=c(0.2,0.2,0.2,0.2))
for (k in length(epsilon):1) {
  for (j in 1:length(u)) {
    Survivaltimes = read.csv(file=paste("./Parameter training rough/u",j," epsilon",k,".csv",sep=""))[,c(6,8)]
