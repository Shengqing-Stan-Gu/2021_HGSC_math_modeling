...
for (k in length(epsilon):1) {
  for (j in 1:length(u)) {
    ...
    predict = read.csv(file=paste("./Parameter training fine/u",j," epsilon",k,".csv",sep=""))[,c(6,8)]
...
write.csv(cbind(length(epsilon):1,Fit),file="./Parameter training fine/Deviation matrix.csv")
