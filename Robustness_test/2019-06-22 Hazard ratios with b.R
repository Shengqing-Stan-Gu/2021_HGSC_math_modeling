...
for (l in 1:length(bseq)) {
  ...
  for (k in length(epsilon):1) {
    for (j in 1:length(u)) {
      Survivaltimes = read.csv(file=paste("./Survival curve/b",l," u",j," epsilon",k,".csv",sep=""))[,c(6,8)]
  ...
  write.csv(HRmatrix,file=paste("./Hazard ratio/HR with b",l,".csv",sep=""))
  write.csv(SurvDiff,file=paste("./Survival difference/SurvDiff with b",l,".csv",sep=""))
}
...
for (l in 1:length(bseq)) {
  pdf(paste("./Survival curve/Survival with b",l,".pdf",sep=""),width=12,height=10)
