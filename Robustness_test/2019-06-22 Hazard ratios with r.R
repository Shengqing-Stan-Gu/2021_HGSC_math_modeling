...
for (l in 1:length(rseq)) {
  ...
  for (k in length(epsilon):1) {
    for (j in 1:length(u)) {
      Survivaltimes = read.csv(file=paste("./Survival curve/r",l," u",j," epsilon",k,".csv",sep=""))[,c(6,8)]
  ...
  write.csv(HRmatrix,file=paste("./Hazard ratio/HR with r",l,".csv",sep=""))
  write.csv(SurvDiff,file=paste("./Survival difference/SurvDiff with r",l,".csv",sep=""))
}
...
for (l in 1:length(rseq)) {
  pdf(paste("./Survival curve/Survival with r",l,".pdf",sep=""),width=12,height=10)
