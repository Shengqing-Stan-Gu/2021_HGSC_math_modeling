...
for (l in 1:length(M1seq)) {
  ...
  for (k in length(epsilon):1) {
    for (j in 1:length(u)) {
      Survivaltimes = read.csv(file=paste("./Survival curve/M1",l," u",j," epsilon",k,".csv",sep=""))[,c(6,8)]
  ...
  write.csv(HRmatrix,file=paste("./Hazard ratio/HR with M1",l,".csv",sep=""))
  write.csv(SurvDiff,file=paste("./Survival difference/SurvDiff with M1",l,".csv",sep=""))
}
...
for (l in 1:length(M1seq)) {
  pdf(paste("./Survival curve/Survival with M1",l,".pdf",sep=""),width=12,height=10)
