...
for (l in 1:length(Mseq)) {
  ...
  for (k in length(epsilon):1) {
    for (j in 1:length(u)) {
      Survivaltimes = read.csv(file=paste("./Survival curve/M",l," u",j," epsilon",k,".csv",sep=""))[,c(6,8)]
  ...
  write.csv(HRmatrix,file=paste("./Hazard ratio/HR with M",l,".csv",sep=""))
  write.csv(SurvDiff,file=paste("./Survival difference/SurvDiff with M",l,".csv",sep=""))
}
...
for (l in 1:length(Mseq)) {
  pdf(paste("./Survival curve/Survival with M",l,".pdf",sep=""),width=12,height=10)
