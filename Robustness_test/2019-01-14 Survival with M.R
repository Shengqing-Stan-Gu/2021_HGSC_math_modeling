...
Mseq = c(4,4.5,5,5.5,6,6.5)
PDSM = matrix(nrow=N,ncol=length(Mseq))
for (i in 1:length(Mseq)) {
  PDSM[,i] = PDSM1/(10^(11.5-Mseq[i]))
}
NACTM = PDSM
...
setwd("~/Desktop/2014-5 Modeling Ovarian Progression/2019-01_Vary_Parameters_M/")
...
for (l in 1:length(Mseq)) {
  y0PDS = read.csv(file="./Expected y at diagnosis/yexpected.csv")[,-1]
  y0NACT = y0PDS
  for (k in 1:length(epsilon)) {
    for (j in 1:length(u)) {
      for (i in 1:N) {
        postPDS = PDStreat(r=r[i],d=d[i],a=a[i],b=b[i],rc=rc[i],dc=dc[i],ac=ac[i],bc=bc[i],epsilon=epsilon[k],M=PDSM[i,l],M1=PDSM1[i],u=u[j],uc=uc[j],y0=y0PDS[i,j])
        postPDSx[i] = postPDS[1]
        postPDSy[i] = postPDS[2]
        if (y0NACT[i,j]*exp((ac[i]-bc[i])*t/2+(a[i]-b[i])*gap1)<M2[i]) {
          postNACT = NACTtreat(r=r[i],d=d[i],a=a[i],b=b[i],rc=rc[i],dc=dc[i],ac=ac[i],bc=bc[i],epsilon=epsilon[k],M=NACTM[i,l],M1=NACTM1[i],u=u[j],uc=uc[j],y0=y0NACT[i,j])
        } else {
        ...
        if (postPDSx[i]+postPDSy[i]<M2[i]) {
          PDSresult = Survive(postPDSx[i],postPDSy[i],r=r[i],d=d[i],rc[i],dc[i],a[i],b[i],ac[i],bc[i],PDSM[i,l],PDSM1[i],M2[i],u[j],uc[j],randomM1,cure=Cure)
        ...
        if (postNACTx[i]+postNACTy[i]<M2[i]) {
          NACTresult = Survive(postNACTx[i],postNACTy[i],r=r[i],d=d[i],rc[i],dc[i],a[i],b[i],ac[i],bc[i],NACTM[i,l],NACTM1[i],M2[i],u[j],uc[j],randomM1,cure=Cure)
      ...
      write.csv(cbind(postPDSx,postPDSy,postNACTx,postNACTy,SurvivalPDS,ChemocountPDS,SurvivalNACT,ChemocountNACT),file=paste("./Survival curve/M",l," u",j," epsilon",k,".csv",sep=""))
    }
  }
}
