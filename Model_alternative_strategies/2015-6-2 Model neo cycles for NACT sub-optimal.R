...
for (i in 1:N) {
  if (y0NACT[i]*exp((ac[i]-bc[i])*t/2+(a[i]-b[i])*gap1)<M2[i]) {
    postNACT = NACTtreat(r=r[i],d=d[i],a=a[i],b=b[i],rc=rc[i],dc=dc[i],ac=ac[i],bc=bc[i],epsilon=epsilon,M=NACTM[i],M1=NACTM1[i],u=u,uc=uc,y0=y0NACT[i],cycle=cycle)
  } else {
...
write.csv(cbind(postNACTx,postNACTy,SurvivalNACT,ChemocountNACT),file="./Survival NACT/Survival 3 cycle sub-optimal.csv")


for (i in 1:N) {
  if (y0NACT[i]*exp((ac[i]-bc[i])*t/2+(a[i]-b[i])*gap1)<M2[i]) {
    postNACT = NACTtreat(r=r[i],d=d[i],a=a[i],b=b[i],rc=rc[i],dc=dc[i],ac=ac[i],bc=bc[i],epsilon=epsilon,M=NACTM[i],M1=NACTM1[i],u=u,uc=uc,y0=y0NACT[i],cycle=cycle2)
  } else {
    postNACT = c(0,y0NACT[i]*exp((ac[i]-bc[i])*t/2+(a[i]-b[i])*gap1))
  }
  postNACTx[i] = postNACT[1]
...
write.csv(cbind(postNACTx,postNACTy,SurvivalNACT,ChemocountNACT),file="./Survival NACT/Survival 2 cycle sub-optimal.csv")


for (i in 1:N) {
  if (y0NACT[i]*exp((ac[i]-bc[i])*t/2+(a[i]-b[i])*gap1)<M2[i]) {
    postNACT = NACTtreat(r=r[i],d=d[i],a=a[i],b=b[i],rc=rc[i],dc=dc[i],ac=ac[i],bc=bc[i],epsilon=epsilon,M=NACTM[i],M1=NACTM1[i],u=u,uc=uc,y0=y0NACT[i],cycle=cycle3)
  } else {
    postNACT = c(0,y0NACT[i]*exp((ac[i]-bc[i])*t/2+(a[i]-b[i])*gap1))
  }
  postNACTx[i] = postNACT[1]
...
write.csv(cbind(postNACTx,postNACTy,SurvivalNACT,ChemocountNACT),file="./Survival NACT/Survival 1 cycle sub-optimal.csv")
